...
$(document).ready(function() {
	var functionJson = '${functionsJson}';
	var functions = JSON.parse(functionJson);
	for(var i = 0; i < functions.length; i++) {
		var cbxId = $("#temp_" + functions[i].functionId).val();
	...
	<ul id="addPanelUl">
		<li>
			<div id="info">
				<ul id="infoUl">
					<li>
						<input type="hidden" name="roleId" value="${roleCustom.roleId }" />
						<span>角色名称：</span>
						<input type="text" name="roleName" value="${roleCustom.roleName }" class="easyui-textbox" data-options="required:true" style="height: 35px;width: 200px;" />
					</li>
					<li>
						<span>角色描述：</span>
						<input type="text" name="des" value="${roleCustom.des }" class="easyui-textbox" style="height: 35px;width: 200px" />
					</li>
				</ul>
			</div>
		</li>
					...
					<c:forEach items="${modules }" var="module">
						<fieldset>
							...
							<c:forEach items="${module.functions }" var="function" varStatus="index">
								<input type="hidden" id="temp_${function.functionId }" value="cbx_${module.moduleId }_${index.index }" />
