...
function beforeRemove(treeId, treeNode) {
	var zTree = $.fn.zTree.getZTreeObj("treeDemo");
	zTree.selectNode(treeNode);
	return confirm("此操作将把该节点以及该分组对应的所有子分组删除！确认删除 分组 -- " + treeNode.name + " 吗？");
}
function onRemove(e, treeId, treeNode) {
	$.ajax({
		url:'${pageContext.request.contextPath}/admin/setting/deleteStLevel.action',
		type:'POST',
		data:{levelId:treeNode.levelId},
...
function onRename(e, treeId, treeNode, isCancel) {
	if(treeNode.name.length != 0) {
		//执行重命名
		$.ajax({
			url:'${pageContext.request.contextPath}/admin/setting/renameStLevel.action',
			type:'POST',
			async: false,
			data:{'levelId':treeNode.levelId, 'newName':treeNode.name},
			success:function(data) {
				var result = JSON.parse(data);
				if(result.msg == "success") {
					setTimeout($.messager.show({
...
function addHoverDom(treeId, treeNode) {
	...
	if (btn) btn.bind("click", function(){
		//添加分组
		$.ajax({
			url:'${pageContext.request.contextPath}/admin/setting/addStLevel.action',
			type:'POST',
			data:{'pid':treeNode.levelId, 'newName':'new node'},
			success:function(data) {
				var result = JSON.parse(data);
				if(result.msg=='success') {
					...
					var successFunc = function() {
						var zTree = $.fn.zTree.getZTreeObj("treeDemo");
						var node = zTree.getNodeByParam("levelId",result.info);
...
function loadTree(successFunc) {
	$.ajax({
		url:"${pageContext.request.contextPath}/admin/setting/getAllStLevel.json",
		type:"POST",
		async: true,
		success:function(data) {
			var result = JSON.parse(data);
			...
			if(msg == 'success') {
				...
				//向tree中添加根
				tree.unshift({levelId:0, pid:-1, name:'根', open:true });
