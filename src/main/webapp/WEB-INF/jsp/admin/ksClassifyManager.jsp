...
function onRemove(e, treeId, treeNode) {
	$.ajax({
		url:'${pageContext.request.contextPath}/admin/setting/deleteKsClassify.action',
		type:'POST',
...
function onRename(e, treeId, treeNode, isCancel) {
	if(treeNode.name.length != 0) {
		//执行重命名
		$.ajax({
			url:'${pageContext.request.contextPath}/admin/setting/renameKsClassify.action',
			type:'POST',
			async: false,
...
function addHoverDom(treeId, treeNode) {
	...
	if (btn) btn.bind("click", function(){
		//添加分组
		$.ajax({
			url:'${pageContext.request.contextPath}/admin/setting/addKsClassify.action',
			type:'POST',
...
function loadTree(successFunc) {
	$.ajax({
		url:"${pageContext.request.contextPath}/admin/setting/getAllKsClassify.json",
		type:"POST",
		async: true,
		success:function(data) {
			var result = JSON.parse(data);
