...
$(document).ready(function (){
	...
	if(type == 'edit') {
		//如果是修改试题，设置试题默认值
		$('#qsnForm').form({
			url : "${pageContext.request.contextPath}/admin/shiti/updateStDanxuan.action",
			onSubmit : function() {
...
function loadStInfo(questionId) {
	...
	$.ajax({
		url:"${pageContext.request.contextPath}/admin/shiti/getStDanxuanById.action",
		dataType:"json",
		loadMsg : '数据装载中......',
		data:{"questionId":questionId},
		success:function(result) {
			if(result.msg == 'success') {
				//加载成功，初始化单选题信息
				var stDanxuan = result.info;
				//设置题目信息
				//alert(stDanxuan.title);
				//alert(stDanxuan.detail);
				var answerTitleEditor = UE.getEditor("answerTitleEditor");
				answerTitleEditor.ready(function() { 
					answerTitleEditor.setContent(stDanxuan.title); 
				});
				//设置题目解析信息
				var answerEditor = UE.getEditor("answerEditor");
				answerEditor.ready(function() { 
					answerEditor.setContent(stDanxuan.detail); 
				});
				//设置选项信息
				var options = stDanxuan.options;
				for(var key in options) {
					//alert("key:" + key);
					//alert("key:"+key+",value:"+options[key].option);
					var liHtml = "<li>"
						+"<input type='radio' id='rdo_" + key + "' name='rightKeyRdo'/>答案&nbsp;&nbsp;"
				...
				//设置正确选项
				var rightKey = stDanxuan.rightKey;
				document.getElementById("rdo_" + rightKey).checked = "checked";
			} else if(result.msg == "error") {
				alert(result.info);
			}
		}
...
function loadBaseInfo() {
	loadCombotree();
	$('#qsnForm').form({
		url : "${pageContext.request.contextPath}/admin/shiti/saveStDanxuan.action",
		onSubmit : function() {
			return $('#qsnForm').form("validate");
		},
		success:function(data) {
...
function addOption() {
	var liHtml = "<li>"
				+"<input type='radio' name='rightKeyRdo'/>答案&nbsp;&nbsp;"
				+"<a class='blue_color_a' href='javascript:;' onclick='removeOption(this)'>删除</a>"
				+"&nbsp;&nbsp;&nbsp;&nbsp;"
...
function saveQsn(type)  {
	...
	//alert("rightKey：" + rightKey);
	//alert("options：" + JSON.stringify(obj));
	//alert("title：" + titleContent);
	//alert("detail：" + detailContent);
...
<div class="easyui-layout" data-options="fit:true" style="width: 100%;height: 100%;">
	<div region="center" style="background: #fafafa;padding:30px;border:0px;font-size: 12px;width: 100%;height: 100%;">
		...
		<div class="easyui-panel"    
			...
			<a class="easyui-linkbutton" iconCls="icon-save" onclick="saveQsn(0);" style="width:130px;height:40px;">保存并继续添加</a>
