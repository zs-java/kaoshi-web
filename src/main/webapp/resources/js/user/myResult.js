$(function() {
	...
	$('#resultTable').bootstrapTable({
        url: baseUrl + '/user/queryMyResult.json',//请求后台的URL（*）
        method: 'POST',                      //请求方式（*）
...
//        pageList: [10, 25, 50, 100],        //可供选择的每页的行数（*）
        ...
        columns: [ {
        	...
        	formatter: function (value, row, index) {
                var pageSize = $('#resultTable').bootstrapTable('getOptions').pageSize;     //通过table的#id 得到每页多少条
                var pageNumber = $('#resultTable').bootstrapTable('getOptions').pageNumber; //通过table的#id 得到当前第几页
            ...
            formatter : function(value, row, index) {
            	if(row.state == 4) //已交卷
            		return "<button class='btn btn-warning' disabled='disabled'>阅卷中</button>";
            	else if (row.state == 5) //已评分
            		return "<button onclick='previewResultById(" + row.id + ");' class='btn btn-info'>查看</button>";
            	else  //错误
            		alert("error");
            }
        }],
...
//查看我的成绩
function previewResultById(kuId) {
	window.open(baseUrl + "/user/previewResult.html?kuId=" + kuId);
}