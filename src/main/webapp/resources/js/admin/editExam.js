...
//	}
	...
	$.ajax({
		url : baseUrl + '/admin/exam/updateExamData.action',
		type : "post",
		data : {
			"ksId" : ksId,
			"name" : examName,
...
//			"userIds" : userIds
		...
		success : function(result) {
			if(result.msg == 'success') {
				...
				msgShow("<span style='color:red;'>" + result.info + "</span>");
			}
		}
	});
}
function loadKsData() {
	$.ajax({
		url : baseUrl + "/admin/exam/getKsDataById.json",
		type : "post",
		data : {"ksId" : ksId}, 
		dataType : "json",
		success : function(data) {
			$("#examName").val(data.name);//考试名称
			$("#examName").validatebox({
				required:true
			});
			$("#examClassify").combotree("setValue", data.ksClassifyId);//考试分类
			var examDescEditor = UE.getEditor('examDesc');//描述 
			examDescEditor.ready(function() {
				examDescEditor.setContent(data.des);
			});
			$("#passScore").val(data.passScore);
			$("#maxTimes").combobox("setValue", data.maxTimes);
			$("#studyTimeStart").datetimebox("setValue", data.beginTimeString);
			$("#studyTimeEnd").datetimebox("setValue", data.endTimeString);
			$("#totalTime").val(data.totalTime);
			$("#pageSize").combobox("setValue", data.pageSize == true ? 1 : 0);
			$("#userSignupFlg").prop("checked", data.userSignupFlg);
			$("#passingAgainFlg").prop("checked", data.passingAgainFlg);
			$("#qsnRandomFlg").prop("checked", data.qsnRandomFlg);
			$("#optionChaosFlg").prop("checked", data.optionRandomFlg);
			$("#unityduration").combobox("setValue", data.timeType ? 1 : 0);
			$("#selectFlg").combobox("setValue", data.selectFlg ? 1 : 0);
			$("#disableexam").val(data.disableTime);
			$("#disablesubmit").val(data.disableSubmit);
			var publishTime = data.resultPublishTimeString;
			if(publishTime.indexOf("1970-01-01") >= 0) {
				$("#rdoNow").prop("checked", true);
				$("#rdoPubTime").prop("checked", false);
			} else {
				$("#rdoNow").prop("checked", false);
				$("#rdoPubTime").prop("checked", true);
				$("#resultPublishTime").datetimebox("setValue", publishTime);
			}
			$("#publishAnswerFlg").prop("checked", data.publishRightkeyFlg);
			$("#isResultRank").prop("checked", data.resultRankFlg);
			if(data.pic != null && data.pic != '') {
				$("#coverPic").attr("src", "/pic/" + data.pic);
			}
			loadSelectSj(data.sjId);
			var ids = [];
			for(var i=0;i<data.userInfoList.length;i++) {
				ids.push(data.userInfoList[i].userId);
			}
			$("#addedUserList").datagrid("load", {
				"ids" : ids
			});
